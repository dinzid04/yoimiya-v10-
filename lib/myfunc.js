const { proto, delay, getContentType, areJidsSameUser, generateWAMessage, jidNormalizedUser } = require("@whiskeysockets/baileys")
...
// [LID FIX] Load LID Database once
...
exports.smsg = (conn, m, store) => {
    ...
    if (m.message) {
        ...
        m.msg = (m.mtype == 'viewOnceMessage' ? m.message[m.mtype].message[getContentType(m.message[m.mtype].message)] : m.message[m.mtype])
                // --- [ BAGIAN 1: FIX PARSING BODY ] ---
        if (m.mtype === 'interactiveResponseMessage') {
            try {
                // Cek Native Flow (Button Modern)
                let msg = m.message.interactiveResponseMessage
                if (msg.nativeFlowResponseMessage) {
                    let json = JSON.parse(msg.nativeFlowResponseMessage.paramsJson)
                    m.body = json.id || json.command || ''
                } 
                // Cek Body Text (Kalau user cuma klik tanpa params)
                else if (msg.body) {
                    m.body = msg.body.text || ''
                }
            } catch (e) {
    ...
    if (m.msg.url) m.download = () => conn.downloadMediaMessage(m.msg)
            // --- [ BAGIAN 2: MASUKKAN BODY KE TEXT ] ---
        m.text = m.msg.text || 
                 m.msg.caption || 
