...
const gameSlot = async (conn, m, db) => {
    ...
    try {
        // Adjusting the probability of winning to 50%
        const winChance = Math.random() < 0.5; // 50% chance to win
        
        if (winChance && slot1 === slot2 && slot2 === slot3) {
            // Winning scenario
            const randomLimit = Math.floor(Math.random() * 3) + 3; // Random limit between 3 and 5
            const randomMoney = Math.floor(Math.random() * 15001) + 5000; // Random money between 5000 and 20000
            
            db.users[m.sender].limit -= 1
            db.set[botNumber].limit += 1
            let sloth =`[  🎰VIRTUAL SLOT 🎰  ]\n•╾━━━━━━━━━━━━━━━━━╼•\n\n${listSlot1}\n${listSlot2}    <<\n${listSlot3}\n\n•╾━━━━━━━━━━━━━━━━━╼•\n[  🎰 VIRTUAL SLOT 🎰  ]\n\n*Keterangan* :\n\`You Win🎉\`\n _Limit:_ +${randomLimit}\n _Uang:_ +${randomMoney}`
