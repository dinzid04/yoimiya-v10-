...
const addPremiumUser = (userId, expired, _dir) => {
    ...
    if (cekUser) {
        cekUser.expired = cekUser.expired + toMs(expired); // Tambahkan waktu jika sudah ada
    } else {
        const obj = { id: userId, expired: Date.now() + toMs(expired) };
        _dir.push(obj); // Tambahkan user baru
    }
    fs.writeFileSync('./database/premium.json', JSON.stringify(_dir, null, 2)); // Simpan ke file
};
